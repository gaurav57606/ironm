...
class _MemberDetailScreenState extends ConsumerState<MemberDetailScreen> {
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          _buildInfoRow('Join Date', AppDateUtils.formatDate(member.joinDate)),
          const Divider(height: 16, color: AppColors.border),
          _buildInfoRow('Expiry', AppDateUtils.formatDate(member.expiryDate ?? DateTime.now()), valueColor: statusColor),
          const Divider(height: 16, color: AppColors.border),
  ...
  Widget _buildActivityHistory(List<Payment> payments, AsyncValue<List<Attendance>> attendanceAsync) {
    return attendanceAsync.when(
      ...
      error: (e, s) => Center(child: Text('Error loading activity')),
      data: (attendance) {
        final sortedPayments = [...payments]..sort((a, b) => b.date.compareTo(a.date));
        
        if (sortedPayments.isEmpty) {
          return Center(child: Padding(padding: EdgeInsets.all(20), child: Text('No payment history', style: AppTextStyles.label)));
        }
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  '${AppDateUtils.formatDate(payment.date)} · ${payment.method.toUpperCase()} · #${payment.invoiceId?.substring(0, 4) ?? "INV"}',
